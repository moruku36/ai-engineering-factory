...
| **永続記憶はリポジトリ中心**<br>*(Git-backed SoT)* | すべての成果物・仕様・検証結果の再現性・監査性を長期的に担保。 | • `main` ブランチがコードおよび仕様（スキーマ・タスク定義）の真実の源泉 (SoT)。<br>• 監査用イベント履歴は現状 `main` の `state/` ディレクトリ配下に投影（`factory/state` ブランチへの分離は🚧Planned）。 |
